...
- **默认值**：`(html) => html`
  ...
  默认的 `html: false` 会将 Markdown 中的原生 HTML 当作普通文本处理，但不会清洗插件生成的 HTML。需要清洗最终输出时，请通过该属性处理。`XSSPlugin` 从 v5 起单独导出且默认不启用，它处理的是 HTML token，不等同于清洗最终输出。参阅[修改 xss 配置](https://imzbf.github.io/md-editor-rt/zh-CN/demo#%F0%9F%94%8F%20%E4%BF%AE%E6%94%B9%20xss%20%E9%85%8D%E7%BD%AE)。
...
- **默认值**：`false`
  ...
  直接在 Markdown 中使用下面的 `<img>` 标签，需要先通过 [markdownItConfig](https://imzbf.github.io/md-editor-rt/zh-CN/api#%F0%9F%8D%A4%20markdownItConfig) 开启 `html: true`。
