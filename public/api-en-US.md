...
- **default**: `(html) => html`
  ...
  The default `html: false` setting treats raw HTML in Markdown as text but does not sanitize HTML generated by plugins. Use this prop to sanitize the final output when needed. `XSSPlugin`, exported separately since v5 and disabled by default, processes HTML tokens rather than sanitizing the final output. Refer to [Modify XSS configuration](https://imzbf.github.io/md-editor-rt/en-US/demo#%F0%9F%94%8F%20Modify%20XSS%20configuration).
...
- **default**: `false`
  ...
  Using the following `<img>` tag directly in Markdown requires enabling `html: true` through [markdownItConfig](https://imzbf.github.io/md-editor-rt/en-US/api#%F0%9F%8D%A4%20markdownItConfig).
...
`MdEditor` and `MdPreview` initialize markdown-it with `{ html: false, breaks: true, linkify: true }` by default. Raw HTML entered in Markdown, such as `<u>`, `<img>`, `<br>`, and `<iframe>`, is displayed as text. HTML in image captions is also displayed as text.

The underline toolbar button and shortcut still insert `<u>text</u>`, which does not render an underline by default. Superscript `^text^`, subscript `~text~`, task lists, Markdown images, code blocks, math, Mermaid, ECharts, and admonitions generate HTML through Markdown rules or plugins and are unaffected by this option.

To enable raw HTML rendering, configure it before creating an editor or preview component:
...
`html` controls raw HTML parsing, while `sanitize` processes the final generated HTML. These settings are independent.
...
The underline shortcut still inserts `<u>text</u>` and requires `html: true` to render an underline.
