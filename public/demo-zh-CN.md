...
原生 HTML 默认通过 `html: false` 关闭，添加 `XSSPlugin` 不会开启 HTML 解析。需要渲染原生 HTML 时，请先通过 [markdownItConfig](https://imzbf.github.io/md-editor-rt/zh-CN/api#%F0%9F%8D%A4%20markdownItConfig) 显式开启。该插件仅处理 HTML token，清洗最终生成的 HTML 请使用 `sanitize`。
