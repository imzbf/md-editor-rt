...
const scrollAuto = (
  pEle: HTMLElement,
  cEle: HTMLElement,
  relatedList: RefObject<SourceLine[]>,
  ...
  const scrollHandler = debounce((e: Event) => {
    ...
    if (e.target === pEle) {
      ...
      for (let i = 0; i < relatedList.current!.length; i++) {
        const { start, end } = relatedList.current![i];
        // // 把当前结束位置定位到下一个开始位置，末尾取自己结束
        const vend =
          i + 1 >= relatedList.current!.length
            ? end
            : relatedList.current![i + 1].start - 1;
      ...
      const { start, end } = relatedList.current![number];
