...
Underline uses the raw HTML tag `<u>`, which is also inserted by the toolbar button and shortcut. With the default `html: false`, it is displayed as text. To render an underline, explicitly enable `html: true` through [markdownItConfig](https://imzbf.github.io/md-editor-rt/en-US/api#%F0%9F%8D%A4%20markdownItConfig).
